...
const SEED_MESSAGES: Message[] = [
  {
    ...
    sender: 'Sarah Chen',
    content:
      "Hey team, can we review the new logo variants for Bloom Studio? I think we're close but want a second opinion on the color palette.",
    time: '11:32 AM',
    isMe: false,
  ...
  },
  {
    ...
    sender: 'You',
    content:
      'Sure, I just uploaded them to the asset library. Thoughts on the color variations? The blue feels more on-brand to me.',
    time: '11:34 AM',
    isMe: true,
  },
  {
    ...
    content:
      "The orange one feels too aggressive. I'd go with the softer blue — it aligns much better with their positioning.",
    time: '11:35 AM',
    isMe: false,
    reactions: [
      { emoji: '✅', count: 3 },
  ...
  },
  {
    ...
    sender: 'You',
    content:
      "Agreed. I'll finalize the blue and create a task to get client sign-off by EOD.",
    time: '11:36 AM',
    isMe: true,
  ...
  },
  {
    ...
    sender: 'Aria Johnson',
    content:
      'Can someone share the brand guidelines doc too? Want to make sure the new logo passes the contrast check.',
    time: '11:40 AM',
    isMe: false,
  },
];
        ...
        {msg.reactions && (
          <div className="mt-1.5 flex items-center gap-1.5">
            {msg.reactions.map((r, i) => (
              <button
                key={i}
                className="flex items-center gap-1 rounded-full border border-border bg-muted px-2 py-0.5 text-[11px] hover:border-primary/40 hover:bg-primary/5 transition-colors"
              >
                <span>{r.emoji}</span>
      ...
      {hovered && (
        <div className="absolute right-4 -top-4 flex items-center gap-0.5 rounded-lg border border-border bg-card px-1 py-0.5 shadow-md z-10">
          {['👍', '✅', '😄'].map((e) => (
            <button
              key={e}
              className="rounded px-1.5 py-1 text-[13px] hover:bg-accent transition-colors"
            >
              {e}
            </button>
          ))}
...
export default function ChatPage() {
  ...
  const [msgMap, setMsgMap] = useState<Record<string, Message[]>>(
    Object.fromEntries(
      CHANNELS.map((c) => [c.id, SEED_MESSAGES.map((m) => ({ ...m }))])
    )
  );
  ...
  const currentMsgs = useMemo(() => {
    return msgMap[activeChannel] ?? [];
  }, [msgMap, activeChannel]);
  ...
  const openTab = (channelId: string) => {
    setFloatingTabs((prev) =>
      prev.find((t) => t.channelId === channelId)
        ? prev.map((t) =>
            t.channelId === channelId ? { ...t, expanded: !t.expanded } : t
          )
  ...
  const toggleTab = (id: string) =>
    setFloatingTabs((p) =>
      p.map((t) => (t.channelId === id ? { ...t, expanded: !t.expanded } : t))
    );
  const closeTab = (id: string) =>
    setFloatingTabs((p) => p.filter((t) => t.channelId !== id));
                  ...
                  <div className="relative">
                    <Avatar className="h-5 w-5">
                      <AvatarFallback
                        ...
                        style={{ backgroundColor: stringToColor(m.id) }}
                      >
                        {getInitials(m.name)}
                      </AvatarFallback>
                    </Avatar>
        ...
        <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex h-14 flex-shrink-0 items-center justify-between border-b border-border px-5">
            ...
            <div className="flex items-center gap-1">
              {(
                [Pin, Users, Search, Phone, Video] as React.ComponentType<{
                  className?: string;
                }>[]
              ).map((Icon, i) => (
                <button
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-0.5">
            ...
            {currentMsgs.map((msg, i) => {
              const showAvatar =
                !currentMsgs[i - 1] || currentMsgs[i - 1].sender !== msg.sender;
              return (
                <MessageBubble key={msg.id} msg={msg} showAvatar={showAvatar} />
              );
            })}
          ...
          <ComposeBox
            ...
            onSend={(t) => sendMsg(activeChannel, t)}
          />
        </div>
      </div>
          ...
          return (
            <div key={tab.channelId} className="flex flex-col items-stretch">
              ...
              >
                ...
                <span className="text-[12.5px] font-medium text-foreground max-w-[90px] truncate">
                  {ch.name}
                </span>
                <ChevronDown
                  className={cn(
        ...
        <button
          onClick={() => {
            const next = CHANNELS.find(
              (c) => !floatingTabs.find((t) => t.channelId === c.id)
            );
