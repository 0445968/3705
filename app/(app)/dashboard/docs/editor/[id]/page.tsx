          ...
          {sizes.map((s) => (
            <button
              ...
              className={cn(
                ...
                size === s ? 'text-primary font-semibold' : 'text-foreground'
              )}
            >
              {s}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(
              ...
              ] as { val: AccessLevel; icon: any; label: string }[]
            ).map(({ val, icon: Icon, label }) => (
              <button
                key={val}
                onClick={() => setAccess(val)}
                className={cn(
                  'flex flex-col items-center gap-1.5 rounded-xl border py-3 transition-all text-[12px] font-medium',
                  access === val
          ...
          <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/20 px-3 py-2">
            <span className="flex-1 text-[12px] font-mono text-muted-foreground truncate">
              {link}
            </span>
            <button
              onClick={copy}
    ...
    <div className="absolute right-0 top-full mt-1 z-50 w-52 rounded-xl border border-border bg-card shadow-xl py-1.5 overflow-hidden">
      ...
      {EXPORT_FORMATS.map((fmt) => (
        <button
          key={fmt.ext}
          onClick={() => {
            alert(`Exporting "${title}" as .${fmt.ext}`);
            onClose();
          }}
...
function ZoomControl({
  ...
  return (
    <div ref={ref} className="relative flex items-center gap-1">
      <button
        onClick={() => onChange(ZOOM_LEVELS[Math.max(0, idx - 1)])}
        disabled={idx === 0}
      ...
      <button
        onClick={() =>
          onChange(ZOOM_LEVELS[Math.min(ZOOM_LEVELS.length - 1, idx + 1)])
        }
        ...
        <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 z-50 w-24 rounded-xl border border-border bg-card shadow-xl py-1 overflow-hidden">
          {ZOOM_LEVELS.map((z) => (
            <button
              ...
              className={cn(
                'flex w-full items-center justify-between px-3 py-1.5 text-[12px] transition-colors hover:bg-accent',
                zoom === z ? 'text-primary font-semibold' : 'text-foreground'
              )}
            >
              {z}%{zoom === z && <Check className="h-3 w-3 text-primary" />}
            </button>
          ))}
        </div>
      )}
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <div className="flex h-12 flex-shrink-0 items-center justify-between border-b border-border bg-card px-4 gap-4 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-[12.5px] text-muted-foreground flex-shrink-0">
            Docs
          </span>
